...
(2026-07-21 진단) 우리 CI/CD는 발표가 "낡은 세계"라 부르는 **PR 중심 모델 그대로**이고, 거기에 AI 헬퍼 3개(리뷰·수리·대시보드)를 볼트온한 상태다. 발표가 "agent 규모가 되면 병목이 드러난다"고 지목한 **중간 지대**에 정확히 서 있다.

> **갱신(2026-07-24):** 이후 ②③④(pre-PR harness)·⑤(merge queue)·B안(pip 캐시)을 구현했다. 아래 표가 최신 상태이며, 이 산문의 "볼트온뿐" 서술은 진단 시점의 것이다. 더 이상 순수 PR 중심이 아니라 harness가 control plane의 일부를 맡는 초기 형태로 이동했다. 종합 as-is/to-be는 `notes/ai/` 참조.
...
(2026-07-21 시점) 발표 프레임으로 요약하면: **human-centric PR 파이프라인 + AI 헬퍼 볼트온.** 이미 노트 02·03이 방향의 씨앗("CI 러너가 agent 실행 환경이 된다", "agent 산출물도 파이프라인에 거주")을 담고 있고, `claude-fix-ci`가 ②④의 초기 형태였다.

**(2026-07-24 갱신)** 이후 `harness/loop.py`로 ②③④를 구현해 "harness가 control plane"의 핵심을 학습 규모로 실장했다. 이제는 "PR 중심 + 볼트온"과 "harness 중심"의 **과도기**다 — inner loop(harness)는 로컬/수동 실행이고, 저장소의 상시 경로는 여전히 PR·CI 게이트다.
...
2. **(2026-07-24 갱신)** 6요소 중 **②③⑤ 구현, ④ 대부분(결정론 게이트 되먹임), ①⑥ 미구현.** 4조건 중 **cache ✅·stateful 🔸**, 속도·병렬 후보탐색은 ❌. (표 참조)
